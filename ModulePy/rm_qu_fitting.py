...
def gridcal_rm_pa(Q, U, freq, rm_left=-10000, rm_right=10000):
...
def mcmc_fit(Q, U, freq, rm_left=-10000, rm_right=10000):
    wave = const.c.value / freq / 1e6
    L = np.sqrt(Q**2 + U**2)
    approx_rm, approx_pa  = gridcal_rm_pa(Q, U, freq, rm_left, rm_right)
    ndim, nwalkers = 2, 10
    p0 = np.zeros((nwalkers, ndim))
    p0[:, 0] = np.random.rand(nwalkers) * 10 + approx_pa
    p0[:, 1] = np.random.rand(nwalkers) * 10 + approx_rm
...
def plot_mcmc_samp(result, save=False):
    ...
    g = sns.kdeplot(data=samp_data, x='PA', y='RM', levels=11, cmap=new_cmap, fill=True, ax=ax1, 
    ...
    plt.ylim(rm_min, rm_max)
    if save:
        plt.savefig('../Figure/RM-QU-Fitting.pdf', format='pdf', dpi=300, bbox_inches='tight')
        plt.close()
    else:
        plt.show()