...
def del_rfi(I, Q, U, V):
    I_profile    = np.mean(I, axis=1)
    I_m          = ArPLS(I_profile, lam=1e4, ratio=0.01, itermax=10)
    X            = I_profile - I_m
    y, x         = np.histogram(X, bins=500, density=True)
    x            = (x[1:] + x[:-1]) / 2
    popt, _      = curve_fit(gaussian, x, y, p0=[1, np.median(X), np.std(X)/1e3, 0])
    labels       = (X < popt[1] - 3 * popt[2]) | (X > popt[1] + 3 * popt[2])
    I[labels, :] = 0
    Q[labels, :] = 0
    U[labels, :] = 0
    V[labels, :] = 0
...
def get_time_index(I):
    ...
    if len(set(signal_time)) <= 2 or len(np.where(signal_time!=0)[0]) < 5:
        ...
        signal_time[np.argmax(np.sum(I, axis=0))-2: np.argmax(np.sum(I, axis=0))+3] = 1
...
#    I, Q, U, V  = del_rfi(I, Q, U, V)
    ...
    rms         = np.random.choice(off_profile, replace=False, size=int(len(off_profile)*0.5)).std()
    snr         = np.sum(I_clean) / np.sqrt(I_clean.shape[1]) / rms
    center_freq = np.average(freq_clean, weights=np.sum(I_clean, axis=1))
    return I_clean, Q_clean, U_clean, V_clean, freq_clean, snr, center_freq